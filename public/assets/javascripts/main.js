(function () {
	...
	function callPlugins (callback) {
	    jquery('[data-plugin]').each(function () {
	        ...
	        var plugins = $element.data('plugin').trim().split(/\s+/);
	...
	function initPlugins () {
	    callPlugins(function ($element, plugin) {
	 ...
	 */
	jquery.fn.examplePlugin = function (data) {
	    if (data === 'destroy') ; else {
	        // Initialize plugin
	        console.log(this, data.xxx);
	    }
	};
	...
	jquery(function () {
	  ...
	  var inputs = jquery(".form__group").find("input, textarea");

	  var inputsData = inputs.map(function (index, input) {
	    var name = jquery(input).attr("name");
	    var errorMessage = jquery(("#" + name + "-error")).text();
	    var type = jquery(input).attr("type");
	    var required = jquery(input).attr("required");
	  ...
	  inputsData.each(function (_, input) {
	    rulesObj[input.name] = {
	...
	jquery(function () {
	  jquery("form").on("submit", function (event) {
	    ...
	    if (isValid) {
	      jquery(".section__form").addClass("fade-out-text");
	  ...
	  var observer = new IntersectionObserver(function (entries) {
	    entries.forEach(function (entry) {
	      if (entry.isIntersecting) {
	        ...
	        jquery(entry.target).addClass('text-jumper');
	      } else {
	        jquery(entry.target).removeClass('text-jumper');
	      }
	    });
	  });

	  jquery('.content__part_text-part').each(function() {
	...
	jquery(function () {
	  var lazyImages = jquery("img.lazyload");
